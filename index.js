...
async function scrapeGoogleMapsReviewsIncremental() {
    console.log(`[${new Date().toISOString()}] Starting incremental Google Maps Review Scrape...`);
    ...
    try {
        ...
        while (true) {
            ...
            for (const r of rawReviews) {
                ...
                if (!seenSignatures.has(uniqueKey) && !newReviewsFoundThisRun.some(nr => `${nr.reviewerName}_${nr.reviewTextOriginal}` === uniqueKey)) {
            ...
            console.log(`Scroll ${scrollIndex}: Found ${newReviewsFoundThisRun.length} valid new reviews so far. Total loaded in DOM: ${rawReviews.length}`);
            ...
            if (consecutiveSeen >= MAX_CONSECUTIVE_SEEN) {
                console.log(`Hit ${consecutiveSeen} consecutive already-saved reviews. Stopping scroll early!`);
                break;
            }
        }
        ...
        console.log(`Writing combined total of ${finalReviews.length} reviews to disk...`);
        ...
        console.log(`[${new Date().toISOString()}] Successfully completed incremental scrape! Added ${formattedNewReviews.length} new reviews.`);

        // Send newly found reviews to Google Apps Script webhook
       if (formattedNewReviews.length > 0) {
    console.log(`Sending ${formattedNewReviews.length} new reviews to Google Apps Script webhook...`);

    try {
        const webhookUrl = process.env.WEBHOOK_URL || 'https://script.google.com/macros/s/AKfycbw5ABwQHdZMsDsDkA_jrdsSBixnHUg5KyIidERy3wAAYtkOuyQIg0mPEraSTf4ODCLi1w/exec';
...
console.log('The scraper will run immediately upon startup, and then every 3 days.');
